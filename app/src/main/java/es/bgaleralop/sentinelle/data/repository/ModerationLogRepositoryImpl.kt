...
class ModerationLogRepositoryImpl(private val logDao: ModerationLogDao) : ModerationLogRepository {
    ...
    override suspend fun updateLogAction(logId: Long, newAction: String) =
        withContext(Dispatchers.IO) {
            logDao.updateLogAction(logId, newAction)
        }

    override suspend fun getChecksCountToday(accountId: Long, startOfDay: Long): Int =
        withContext(Dispatchers.IO) {
            logDao.getChecksCountToday(accountId, startOfDay)
        }

    override suspend fun getSpamCountToday(accountId: Long, startOfDay: Long): Int =
        withContext(Dispatchers.IO) {
            logDao.getSpamCountToday(accountId, startOfDay)
        }

    override fun getMostActiveSpamWords(accountId: Long): Flow<List<WordStat>> =
        logDao.getMostActiveSpamWords(accountId).flowOn(Dispatchers.IO)
}